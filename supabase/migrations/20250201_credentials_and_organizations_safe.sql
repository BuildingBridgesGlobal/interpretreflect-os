-- Organizations and Credentials System (Safe Version)
-- Migration: 20250201_credentials_and_organizations_safe
-- This version uses DROP IF EXISTS to handle partial application

-- Drop existing triggers first (safe)
DROP TRIGGER IF EXISTS set_credential_organization_trigger ON credentials;
DROP TRIGGER IF EXISTS update_credential_status_trigger ON credentials;
...
-- Add organization_id and role to profiles
ALTER TABLE profiles
ADD COLUMN IF NOT EXISTS organization_id UUID REFERENCES organizations(id) ON DELETE SET NULL;

ALTER TABLE profiles
ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'interpreter';
...
-- Create function to get all credentials for an organization
...
-- Create function to get credential stats
...
DROP POLICY IF EXISTS "Admins can view organization credentials" ON credentials;
...
COMMENT ON COLUMN credentials.status IS 'Auto-calculated based on expiration_date';
