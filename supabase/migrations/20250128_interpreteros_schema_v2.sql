-- InterpreterOS Database Schema v2
-- This migration handles existing tables and adds new ones

-- ============================================================================
-- DROP EXISTING SKILLS TABLE IF IT EXISTS (to recreate with proper schema)
-- ============================================================================
DROP TABLE IF EXISTS skill_assessments CASCADE;
DROP TABLE IF EXISTS skill_goals CASCADE;
DROP TABLE IF EXISTS training_module_skills CASCADE;
...
CREATE TABLE training_module_skills (
...
DROP POLICY IF EXISTS "Skills are viewable by all authenticated users" ON skills;
...
DROP POLICY IF EXISTS "Users can view their own skill assessments" ON skill_assessments;
CREATE POLICY "Users can view their own skill assessments"
  ON skill_assessments FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own skill assessments" ON skill_assessments;
...
DROP POLICY IF EXISTS "Users can view their own skill goals" ON skill_goals;
CREATE POLICY "Users can view their own skill goals"
  ON skill_goals FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own skill goals" ON skill_goals;
CREATE POLICY "Users can insert their own skill goals"
  ON skill_goals FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own skill goals" ON skill_goals;
...
DROP POLICY IF EXISTS "Training modules are viewable by all authenticated users" ON training_modules;
CREATE POLICY "Training modules are viewable by all authenticated users"
  ON training_modules FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Training module skills are viewable by all authenticated users" ON training_module_skills;
...
DROP POLICY IF EXISTS "Users can view their own training assignments" ON training_assignments;
CREATE POLICY "Users can view their own training assignments"
  ON training_assignments FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own training assignments" ON training_assignments;
...
DROP POLICY IF EXISTS "Users can insert their own debriefs" ON debriefs;
...
-- Debrief skills
DROP POLICY IF EXISTS "Users can view debrief skills for their debriefs" ON debrief_skills;
...
-- Performance flags
DROP POLICY IF EXISTS "Users can view performance flags for their debriefs" ON performance_flags;
...
DROP POLICY IF EXISTS "Users can view their own elya messages" ON elya_messages;
CREATE POLICY "Users can view their own elya messages"
  ON elya_messages FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own elya messages" ON elya_messages;
