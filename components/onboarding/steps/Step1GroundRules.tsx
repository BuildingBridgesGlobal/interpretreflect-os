...
export default function Step1GroundRules({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <div className="space-y-4">
      <p className="text-sm text-slate-300">Your reflections stay private. Teams and programs only see anonymized patterns.</p>
      <p className="text-sm text-slate-300">No ads, no selling your journal, ever.</p>
      ...
      <label className="mt-4 flex items-center gap-2 text-sm text-slate-200">
        ...
        I understand this OS is for support and reflection, not emergency care.
      </label>
    </div>
  );
}
